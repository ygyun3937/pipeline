...
동작 흐름:
    ...
    4. LLMClient를 호출하여 구체화 결과를 생성한다.
...
class IssueElaborator:
    """
    LLMClient를 사용하여 모호한 이슈를 구체화하는 클래스.

    IssueRetriever로 유사 과거 이슈를 검색한 뒤,
    ELABORATION_SYSTEM_PROMPT와 함께 LLM에게 구체화를 요청한다.
    """
    ...
    def __init__(
        ...
        logger.info(
            "IssueElaborator 초기화: model=%s, max_retries=%d",
            self.model_name,
            max_retries,
        )
    ...
    async def _generate_with_retry(self, user_message: str) -> str:
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                return await self._llm.complete(ELABORATION_SYSTEM_PROMPT, user_message)
