...
class QAReportGenerator:
    """
    LLMClient를 사용하여 QA 리포트를 생성하는 클래스.

    ElaborationResult(Stage 1), FeasibilityResult(Stage 2), TestResultSet을 입력받아
    REPORT_SYSTEM_PROMPT와 함께 LLM에게 리포트 생성을 요청하고
    ...
    def __init__(
        ...
        logger.info(
            "QAReportGenerator 초기화: model=%s, reports_dir=%s, max_retries=%d",
            self.model_name,
            self._reports_dir,
            max_retries,
        )
    ...
    async def _generate_with_retry(self, user_message: str) -> str:
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                return await self._llm.complete(REPORT_SYSTEM_PROMPT, user_message)
