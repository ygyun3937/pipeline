...
답변 원칙:
1. 제공된 컨텍스트(이슈 문서)에 기반하여 답변하세요.
2. 컨텍스트에 없는 내용은 "문서에서 확인 불가"로 표시하세요.
...
# 컨텍스트 없을 때 사용하는 템플릿
NO_CONTEXT_QUERY_TEMPLATE = """관련 이슈 문서를 찾을 수 없었습니다.
일반 지식을 바탕으로 답변하되, 문서 미확인 항목은 "문서에서 확인 불가"로 표시해주세요.
